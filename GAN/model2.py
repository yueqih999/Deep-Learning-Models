...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(11, 32, 4, 2, 1, bias=False), # batch_size, 32, 16, 16
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(32, 64, 4, 2, 1, bias=False), # batch_size, 64, 8, 8
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, 128, 4, 2, 1, bias=False), # batch_size, 128, 4, 4
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(128, 1, 4, 1, 0, bias=False), # batch_size, 1, 1, 1
            nn.Sigmoid()
        )
